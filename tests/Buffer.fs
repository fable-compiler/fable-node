  ...
  testList "Buffer" [
    ...
    testList "buf" [
      ...
      testCase "buf.writeXX " <| fun _ ->
        ...
        let buf = Buffer.allocUnsafe 4
        buf.writeUInt32BE(float 0xfeedfaceL,0) |> ignore
        buf.writeUInt32LE(float 0xfeedfaceL,0) |> ignore
        let buf = Buffer.allocUnsafe 6
        buf.writeUIntBE(float 0x1234567890abL, 0, 6) |> ignore
        buf.writeUIntLE(float 0x1234567890abL, 0, 6) |> ignore
        let buf = Buffer.allocUnsafe 6
        buf.writeIntBE(float 0x1234567890abL,0,6) |> ignore
        buf.writeIntLE(float 0x1234567890abL,0,6) |> ignore
        testPassed()

...
type [<AllowNullLiteral>] ReadableOptions =
    ...
    /// The size argument is advisory. For implementations where a "read" is a single operation that returns data can use the size argument to determine how much data to fetch. Other implementations may ignore this argument and simply provide data whenever it becomes available. There is no need to "wait" until size bytes are available before calling push(chunk).
...
type [<AllowNullLiteral>] Readable<'a> =
    ...
    /// When chunk is a Buffer, byte[] or string, the chunk of data will be added to the internal queue for users of the stream to consume. Passing chunk as null signals the end of the stream (EOF), after which no more data can be written.
    ...
    abstract push: chunk: 'a option * ?encoding: BufferEncoding -> bool
    /// The readable.unshift() method pushes a chunk of data back into the internal  This is useful in certain situations where a stream is being consumed by code that needs to "un-consume" some amount of data that it has optimistically pulled out of the source, so that the data can be passed on to some other party.
    /// Note: The unshift(chunk) method cannot be called after the 'end' event has been emitted or a runtime error will be thrown.
    /// Developers using unshift() often should consider switching to use of a Transform stream instead.
    abstract unshift: chunk: 'a -> unit
    /// The readable.pipe() method attaches a Writable stream to the readable, causing it to switch automatically into flowing mode and push all of its data to the attached Writable. The flow of data will be automatically managed so that the destination Writable stream is not overwhelmed by a faster Readable
    ...
    /// The readable.pause() method will cause a stream in flowing mode to stop emitting 'data' events, switching out of flowing mode. Any data that becomes available will remain in the internal
    ...
    /// The readable.setEncoding() method sets the character encoding for data read from the Readable
    ...
    abstract setEncoding: BufferEncoding -> unit
...
type [<AllowNullLiteral>] WritableOptions<'a> =
    inherit Options
    /// Whether or not to decode strings into Buffers before passing them to _write(). Defaults to true
    ...
    abstract write: ('a -> BufferEncoding -> (Error option -> unit) -> unit) option with get, set
...
type [<AllowNullLiteral>] Writable<'a> =
    ...
    abstract write: chunk: 'a * encoding: BufferEncoding * ?cb: (Error option -> unit) -> bool
    abstract write: chunk: 'a * cb: (Error option -> unit) -> bool
    /// Calling the writable.end() method signals that no more data will be written to the Writable. The optional chunk and encoding arguments allow one final additional chunk of data to be written immediately before closing the  If provided, the optional callback ('FIn->'FOut) is attached as a listener for the 'finish' event.
    /// Calling the write() method after calling end() will raise an error.
    abstract ``end``: cb: unit -> unit
    abstract ``end``: chunk: 'a -> unit
    abstract ``end``: chunk: 'a * cb: (unit -> unit) -> unit
    abstract ``end``: chunk: 'a * encoding: BufferEncoding * ?cb: (unit -> unit) -> unit
...
type [<AllowNullLiteral>] DuplexOptions<'a> =
    ...
    /// Defaults to false. Sets objectMode for readable side of the  Has no effect if objectMode is true.
    abstract readableObjectMode: bool option with get, set
    /// Defaults to false. Sets objectMode for writable side of the  Has no effect if objectMode is true.
...
type [<AllowNullLiteral>] TransformOptions<'a, 'b> =
    ...
    /// In some cases, a transform operation may need to emit an additional bit of data at the end of the  For example, a zlib compression stream will store an amount of internal state used to optimally compress the output. When the stream ends, however, that additional data needs to be flushed so that the compressed data will be complete.
    /// Custom Transform implementations may implement the transform._flush() method. This will be called when there is no more written data to be consumed, but before the 'end' event is emitted signaling the end of the Readable
    /// Within the transform._flush() implementation, the readable.push() method may be called zero or more times, as appropriate. The callback ('FIn->'FOut) must be called when the flush operation is complete.
